...
class Experiments():
    def __init__(self, dist, sys_sep, algo, lin,lin_t, lin_const, random_choice, true_prob, sample_size, repeats, gap, gd_eps,dp, dp_eps):
    ...
    def set_path(self):
        cwdname = os.getcwd()
        if self.dp:
            folder = cwdname + self.sys_sep + "result" + self.sys_sep + self.dist + self.sys_sep + str(self.sample_size) + self.sys_sep +"Arm_" + str( len(self.true_prob) ) + self.sys_sep + self.random_choice + self.sys_sep + "experiments_"+ self.algo + "_dp" + str(self.dp_eps) + "_" + self.dist
        elif self.lin:
            folder = cwdname + self.sys_sep + "result" + self.sys_sep+ self.dist + self.sys_sep + str(self.sample_size)  + self.sys_sep +"Arm_" + str( len(self.true_prob) ) + self.sys_sep + self.random_choice + self.sys_sep + "experiments_"+ self.algo + "_linear"+"_" + self.dist
        elif self.lin_t:
            folder = cwdname + self.sys_sep + "result" + self.sys_sep+ self.dist + self.sys_sep + str(self.sample_size)  + self.sys_sep +"Arm_" + str( len(self.true_prob) ) + self.sys_sep + self.random_choice + self.sys_sep + "experiments_"+ self.algo + "_linear_const"+"_" + self.dist
        else:
            folder = cwdname + self.sys_sep + "result" + self.sys_sep+ self.dist + self.sys_sep + str(self.sample_size)  + self.sys_sep +"Arm_" + str( len(self.true_prob) ) + self.sys_sep + self.random_choice + self.sys_sep + "experiments_"+ self.algo + "_original"+"_" + self.dist
    ...
    def run_experiments(self):
        ...
        for j in range(self.repeats):
            bandits = Bandits(random_choice= self.random_choice, true_prob=self.true_prob, sample_size=self.sample_size, algo = self.algo, lin = self.lin, 
                              gd_eps=self.gd_eps,lin_t=self.lin_t, lin_const=self.lin_const, dp= self.dp, dp_eps=self.dp_eps)
            (bandit_stats, output_bias, output_means, output_variance, output_pulls, output_cov, output_corr, output_reward) = bandits.run()
            #return self.bandit_stats, self.output_bias, self.output_means, self.output_pulls, self.output_cov, self.output_corr, self.output_reward
        ...
        return mix_dict_bias, mix_dict_means, mix_dict_variance, mix_dict_pulls,mix_dict_pulls2, mix_dict_cov, mix_dict_corr, a, mix_dict_upper, mix_dict_lower, mix_dict_mean2, mix_dict_ci_mark, mix_dict_p_mark1,mix_dict_p_mark2, mix_dict_var, mix_dict_z2, mix_dict_t, mix_dict_df_ci_mark


    def compute_total_stats(self):
        (mix_dict_bias, mix_dict_means, mix_dict_variance,  mix_dict_pulls,mix_dict_pulls2, mix_dict_cov,mix_dict_corr, a, mix_dict_upper, mix_dict_lower, mix_dict_mean2, mix_dict_ci_mark, mix_dict_p_mark1,mix_dict_p_mark2, mix_dict_var, mix_dict_z2, mix_dict_t, mix_dict_df_ci_mark) = self.run_experiments()
        ...
        return mix_dict_bias, mix_dict_means, mix_dict_variance,  mix_dict_pulls,mix_dict_pulls2, mix_dict_cov, mix_dict_corr, total_stats, mix_dict_mean2, mix_dict_ci_mark,mix_dict_var, mix_dict_z2, mix_dict_t, mix_dict_lower, mix_dict_upper, mix_dict_df_ci_mark


    def total_outputs(self):
        (mix_dict_bias, mix_dict_means, mix_dict_variance, mix_dict_pulls,mix_dict_pulls2, mix_dict_cov, mix_dict_corr, total_stats, mix_dict_mean2, mix_dict_ci_mark,mix_dict_var, mix_dict_z2, mix_dict_t, mix_dict_lower, mix_dict_upper, mix_dict_df_ci_mark) = self.compute_total_stats()
